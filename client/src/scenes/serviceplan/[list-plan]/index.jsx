...
const ServicePlans = () => {
  ...
  const handleEditSubmit = async () => {
    try {
      await axios.put(
        `http://localhost:5000/api/admin/packages/${editData.id}`,
        editData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <Box sx={{ minHeight: "100vh", p: 4, color: "white" }}>
            ...
            <Grid item xs={12} sm={6} md={4} key={pkg.id}>
              <Card
                sx={{
                  ...
                  border:
                    index === 1 ? "2px solid #007BFF" : "1px solid #415A77",
      ...
      <Dialog open={!!viewPlan} onClose={() => setViewPlan(null)}>
        ...
        <DialogActions>
          <Button onClick={() => setViewPlan(null)} variant="contained">
            Close
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={!!editPlan} onClose={() => setEditPlan(null)}>
          ...
          {editPlan &&
            Object.keys(initialState).map((key) => (
              <TextField
                ...
                type={
                  ["price", "duration", "max_sites", "max_ponds", "max_workers"].includes(key)
                    ? "number"
                    : "text"
                }
                sx={{ mb: 2 }}
        ...
        <DialogActions>
          <Button onClick={() => setEditPlan(null)} color="secondary">
            Cancel
          </Button>
