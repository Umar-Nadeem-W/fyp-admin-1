...
const CustomerQueries = () => {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("customerQueries");
    if (stored) {
      ...
      setQueries(defaultData);
      localStorage.setItem("customerQueries", JSON.stringify(defaultData));
    }
  }, []);
    ...
    <Box sx={{ padding: 3 }}>
      ...
      >
        ...
        <Box>
          ...
          >
            {["All", "Today", "Last Week", "Last Month"].map((option) => (
              <MenuItem key={option} onClick={() => handleDateFilterClose(option)}>
                {option}
              </MenuItem>
            ))}
          </Menu>
          ...
          >
            {["All", "Pending", "Handled"].map((option) => (
              <MenuItem key={option} onClick={() => handleStatusFilterClose(option)}>
                {option}
              </MenuItem>
            ))}
          </Menu>
