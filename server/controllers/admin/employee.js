...
export const getAllEmployees = async (req, res) => {
  try {
    const [employees] = await db.execute(`
      SELECT e.*, u.user_name, u.email, u.contact_number, e.designation, e.status,
             e.manage_devices, e.send_announcement, e.manage_users, e.can_see_complaints AS employee_name
      FROM employee e
...
export const addEmployee = async (req, res) => {
  ...
  try {
    ...
    const [user] = await db.execute("SELECT * FROM user WHERE id = ?", [u_id]);

    if (user.length === 0) {
      return res.status(404).json({ error: "User not found" });
    }

    // Fallback for undefined fields (MySQL does not accept undefined)
    designation = designation ?? null;
    status = status ?? "not approved"; // fallback if missing
